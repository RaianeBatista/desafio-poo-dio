...
Desafio este que foi elaborado pelo professora Camila Cavalcante na plataforma de cursos online **DIGITAL INNOVATION ONE** 🧡💛 e disponibilizado de forma gratuita para a comunidade dos desenvolvedores Java.
...
A visão de Orientação a Objetos (OO) é aquela de um mundo de objetos que interagem. Este paradigma é um modelo de análise, projeto e programação baseado na aproximação entre o mundo real e o mundo virtual, através da criação e interação entre classes, atributos, métodos, objetos, entre outros.
...
* **🔻 OBJETO:** Em POO, objeto é um "molde" de uma determinada classe, que passa a existir a partir de uma instância da classe. A classe define o comportamento do objeto, usando atributos (propriedades) e métodos (ações). Objeto em ciência da computação, é uma referência a um local da memória que possui um valor.
...
O programa descreve o processamento necessário para solucionar o problema. Caracterizado por execução sequencial, uso de variáveis e instruções de atribuição.
...
Modelo no qual os resultados são descritos, mas os passos para chegar aos resultados não são estabelecidos.
...
Disponibilizado com ❤️ por [RaianeBatista](https://github.com/RaianeBatista).